...
;; zie deel 1c p. 21
;;
(define analyze-amb (lambda ((<update> exp expression))
  (let ((cprocs (map analyze (amb-choices (<update> exp expression)))))
...
(define input (list 'begin
                    ...
                    (list 'define '(multiple-dwelling)
                      (list 'let '((baker    (amb 1 2 3 4 5))
                        ...
                        (list 'require (list 'not (list '= 'baker (<update> 5 4))))
                        '(require (not (= cooper 1)))
                        '(require (not (= fletcher 5)))
                        '(require (not (= fletcher 1)))
                        '(require (> miller cooper))
                        (list 'require (list 'not (list '= '(abs (- smith fletcher)) (<update> 1 2))))
                        '(require (not (= (abs (- fletcher cooper)) 1)))
                        (<update> #t '(require (< (abs (- miller smith)) 3)))
