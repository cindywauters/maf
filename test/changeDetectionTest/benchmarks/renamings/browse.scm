...
(define (put (<update> key1 firstKey) (<update> key2 secondKey) val)
  (let ((x (lookup (<update> key1 firstKey) properties)))
    (if x
        (let ((y (lookup (<update> key2 secondKey) (cdr x))))
          (if y
              (set-cdr! y val)
              (set-cdr! x (cons (cons (<update> key2 secondKey) val) (cdr x)))))
        (set! properties
              (cons (list (<update> key1 firstKey) (cons (<update> key2 secondKey) val)) properties)))))
