...
(define (rand)
  (let* (((<update> hi high) (quotient (car *seed*) 127773))
         ((<update> lo low) (modulo (car *seed*) 127773))
         (test (- (* 16807 (<update> lo low)) (* 2836 (<update> hi high)))))
     (if (> test 0)
        (set-car! *seed* test)
...
(define (cartesian-multipole-to-local-convert pt multipole-expansion)
  (define pt-expansion
    (let* ((one-radius (/ (pt-r vect)))
	   (two-cosines (pt-scalar* (* 2 one-radius) vect))
	   (x (pt-x two-cosines))
	   (y (pt-y two-cosines))
	   (z (pt-z two-cosines)))
      (make-cartesian-expansion
