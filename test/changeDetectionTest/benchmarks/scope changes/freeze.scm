...
;; zie deel 1.1 p46
;;
(<insert> (define (lookup-variable-value var env)
  (define (env-loop env)
...
;;
(define (eval exp env)
(let ((<delete> (lookup-variable-value (lambda (var env)
        (define (env-loop env)
          (define (scan vars vals)
            (cond ((null? vars)
                   (env-loop (enclosing-environment env)))
  ...
  (cond ((self-evaluating? exp) exp)
        ...
        ((<insert> (freeze? exp)) (eval-freeze exp env))
        ((application? exp)
...
(define (frame-variables frame) (<update> (car frame) (vector-ref frame 0)))
(define (frame-values frame) (<update> (cdr frame) (vector-ref frame 1)))
(<insert> (define frame-frozen?s (lambda (frame) (vector-ref frame 2))))

(define (add-binding-to-frame! var val frame)
  (<update> (begin ; Added `begin`s to accomodate change expression.
