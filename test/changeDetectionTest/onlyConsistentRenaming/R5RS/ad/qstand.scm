...
(define quick-sort (lambda (vector)
      (define swap (<change>
            ...
            (lambda (_low0 _high0)
               (define quick-sort-aux-iter (lambda (_mid-value0 _from0 _to0)
                     (define quick-right (lambda (_index10)
                           (if (< (vector-ref vector _index10) _mid-value0)
                              (quick-right (+ _index10 1))
                              _index10)))
                     (define quick-left (lambda (_index20)
                           (if (> (vector-ref vector _index20) _mid-value0)
               ...
               (if (< _low0 _high0)
                  (begin
                     (if (> (vector-ref vector _low0) (vector-ref vector _high0))
                        (swap vector _low0 _high0)
                        #f)
                     (let ((_mid-index0 (quick-sort-aux-iter (vector-ref vector _low0) _low0 _high0)))
                        (swap vector _mid-index0 _low0)
