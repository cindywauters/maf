...
(define quick-sort (lambda (vector)
      (define swap (lambda (v index1 index2)
            (<change>
               ...
               (let ((_temp0 (vector-ref v index1)))
                  (vector-set! v index1 (vector-ref v index2))
                  (vector-set! v index2 _temp0)))))
            ...
            (define quick-sort-aux-iter (lambda (mid-value from to)
                  (define quick-right (lambda (index1)
                        (if (if (< index1 high) (< (vector-ref vector index1) mid-value) #f)
                           ...
                           index1)))
                  (define quick-left (<change>
                        (lambda (index2)
                           (if (if (> index2 low) (> (vector-ref vector index2) mid-value) #f)
                              (quick-left (- index2 1))
                              index2))
                        (lambda (_index20)
                           (if (if (> _index20 low) (> (vector-ref vector _index20) mid-value) #f)
                  ...
                  (<change>
                     (let ((index1 (quick-right (+ from 1)))
                        ...
                        (if (< index1 index2)
                           ...
                           index2))
                     (let ((_index10 (quick-right (+ from 1)))
                           (_index20 (quick-left to)))
                        (if (< _index10 _index20)
                           (begin
                              (swap vector _index10 _index20)
                              (quick-sort-aux-iter mid-value _index10 _index20))
                           _index20)))))
            (if (< low high)
